...
def verificar_saude_api(timeout_s: float = 15.0) -> None:
    """Health check rapido antes de comecar o processamento pesado: uma
    chamada minima (1 token) com timeout curto e FIXO (nao usa o timeout_s
    do YAML, que pode ser 120s -- aqui queremos falhar em segundos, nao
    minutos). Sem isso, uma falha de API bem no inicio do job (ex. o 404
    isolado que travou o job 17800 por 18h) so aparece muito mais tarde,
    as vezes travando o processo sem nunca reportar nada de novo no log.

    Aborta o job (RuntimeError) se a API nao responder um chat completion
    valido dentro do timeout. Imprime timestamp absoluto para dar um
    ponto de referencia claro no log."""
    import datetime
    agora = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"[{agora}] Verificando saude da API em {BASE_URL} (timeout={timeout_s}s)...", flush=True)
    try:
        client = get_client()
        ...
        agora = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f"[{agora}] API respondeu OK (health check).", flush=True)
    except (APIStatusError, APIConnectionError) as e:
        raise RuntimeError(
            f"Health check falhou: a API em {BASE_URL} nao respondeu "
            f"corretamente antes mesmo de comecar o processamento. "
            f"Abortando cedo para nao gastar horas de cluster. "
...
def chamar_llm(client: OpenAI, system_prompt: str, user_content: str,
    ...
    try:
        ...
        # Erro HTTP explicito (404, 401, 429, 5xx...). O SDK ja fez as
        # `tentativas` configuradas antes de chegar aqui. Reempacotamos com
        # a URL completa e o corpo cru da resposta, porque o str(e) padrao
        # do SDK costuma truncar isso e some no meio de um print generico.
        url_completa = getattr(getattr(e, "request", None), "url", "desconhecida")
        try:
            corpo = e.response.text
        except Exception:
            corpo = repr(getattr(e, "body", None))
        raise RuntimeError(
            f"Erro HTTP {e.status_code} chamando {url_completa} "
            f"(modelo={LLM_CFG['modelo']}). Corpo da resposta: {corpo[:2000]!r}"
        ) from e
    except APIConnectionError as e:
        # Falha de rede/DNS/timeout antes mesmo de obter uma resposta HTTP
        # (diferente de APIStatusError, que exige que o servidor tenha
        # respondido algo, mesmo que seja um erro).
        raise RuntimeError(
            f"Falha de conexao chamando {BASE_URL} (modelo={LLM_CFG['modelo']}): {e}"
        ) from e
...
def _extrair_uma_amostra(client, system_prompt, abstract_texto):
    ...
    (erro de API/rede ou JSON invalido -- ver `_ULTIMO_ERRO_FOI_API` para
    diferenciar dos dois casos, usado por rodar_prompt_sobre_abstracts para
    contar erros de API consecutivos)."""
    import time
    import datetime
    global _ULTIMO_ERRO_FOI_API
    t = time.time()
    try:
        raw = chamar_llm(client, system_prompt, abstract_texto)
        agora = datetime.datetime.now().strftime("%H:%M:%S")
        print(f"    [{agora} | ok em {time.time()-t:.1f}s | prompt={len(system_prompt)} chars | abstract={len(abstract_texto)} chars]", flush=True)
        _ULTIMO_ERRO_FOI_API = False
        data = json.loads(raw)
        ...
        # RuntimeError aqui cobre o reempacotamento feito em chamar_llm
        # (erro HTTP com URL/corpo, ou "API nao retornou conteudo").
        _ULTIMO_ERRO_FOI_API = True
        agora = datetime.datetime.now().strftime("%H:%M:%S")
        print(f"    [{agora} | ERRO DE API em {time.time()-t:.1f}s | prompt={len(system_prompt)} chars | abstract={len(abstract_texto)} chars | {type(e).__name__}: {e}]", flush=True)
        return None
    except Exception as e:
        # Erro de parsing/JSON malformado etc: nao conta como falha de API.
        _ULTIMO_ERRO_FOI_API = False
        agora = datetime.datetime.now().strftime("%H:%M:%S")
        print(f"    [{agora} | erro em {time.time()-t:.1f}s | prompt={len(system_prompt)} chars | abstract={len(abstract_texto)} chars | {type(e).__name__}: {e}]", flush=True)
        return None
...
def rodar_prompt_sobre_abstracts(client: OpenAI, system_prompt: str,
    ...
    Aborta cedo (RuntimeError) se acumular LIMITE_ERROS_CONSECUTIVOS erros
    de API seguidos: indica que o servico caiu (ex. 404/5xx sistemico) em
    vez de falhas pontuais por abstract, e continuar so gastaria horas de
    cluster gravando extracoes vazias para o resto do corpus."""
    ...
    with saida.open("a", encoding="utf-8") as f:
        for doc_id, texto in abstracts.items():
            ...
            if _ULTIMO_ERRO_FOI_API:
                erros_consecutivos += 1
                if erros_consecutivos >= LIMITE_ERROS_CONSECUTIVOS:
                    raise RuntimeError(
                        f"Abortando: {erros_consecutivos} erros de API consecutivos "
                        f"(o ultimo foi em doc_id={doc_id!r}). Isso indica que o "
                        f"servico/proxy caiu (ex. 404 sistemico), nao falha pontual "
                        f"de um abstract -- ver mensagem de erro acima para a URL e "
                        f"o corpo da resposta. {saida} ja tem {len(feitos)} doc_id(s) "
                        f"gravados e pode ser retomado depois que o servico voltar."
                    )
            else:
                erros_consecutivos = 0
...
def main():
    ...
    print(f"[config] modelo={cfg['modelo']} temperatura={cfg['temperatura']} "
          ...
          f"k={cfg['k_autoconsistencia']}", flush=True)
    verificar_saude_api()
    args.func(args)
